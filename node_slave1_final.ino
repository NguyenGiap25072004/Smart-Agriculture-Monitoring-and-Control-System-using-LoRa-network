...
void Processing_incoming_data() {
    // Đảo ngược logic (!digitalRead) vì Relay là Active LOW
    // Đọc mức LOW (0) -> Phủ định thành 1 (Gửi về Web là ON)
    // Đọc mức HIGH (1) -> Phủ định thành 0 (Gửi về Web là OFF)
...
void Processing_incoming_data_for_Ctrl_LEDs() {
  ...
  if (LED_Num == "1") {
    // Nếu lệnh là "t" (true/on) -> Xuất mức LOW để đóng Relay
    if (LED_State == "t") digitalWrite(LED_1_Pin, LOW); 
    if (LED_State == "f") digitalWrite(LED_1_Pin, HIGH);
  }

  if (LED_Num == "2") {
    if (LED_State == "t") digitalWrite(LED_2_Pin, LOW);
    if (LED_State == "f") digitalWrite(LED_2_Pin, HIGH);
  }
}
...
// Bổ sung: ÉP RELAY TẮT NGAY KHI KHỞI ĐỘNG
...
void loop() {
  ...
  if (currentMillis_UpdateDHT11 - previousMillis_UpdateDHT11 >= interval_UpdateDHT11) {
    ...
    // Đọc cảm biến vi khí hậu (Nhiệt độ, Độ ẩm, Ánh sáng)
    ...
    // Quy trình nội suy giá trị độ ẩm đất (Soil Moisture Calibration)
    // Giá trị thực nghiệm ADC: 4095 (Mốc khô tuyệt đối) -> 0 (Mốc ngập nước)
    // Chuyển đổi thành dải phần trăm: 0% -> 100%
